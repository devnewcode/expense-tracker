...
export default function Main(){
    ...
    useEffect(()=>{
        ...
        allTransactions.forEach(item=>{
            ...
            expense = expense = expense + parseFloat(item.amount)
        })
    ...
    return (
        <Flex textAlign={"center"} flexDirection={"column"} pr={"5"} pl={"5"} >
            ...
            <ExpenseView 
            data={allTransactions.filter(item=> item.type === 'expense')} type={'expense'} />
            <ExpenseView 
            data={allTransactions.filter(item=> item.type === 'income')} type={'income'} />
            </Flex>
        </Flex>
    )
}